...
def light_offsets(y, x, grid_, zeroed):
    global offsets, total_flashes
    for o in offsets:
        if in_bounds(y, x, o):
            if (y + o[0], x + o[1]) not in zeroed:
                ...
                if grid_[y + o[0]][x + o[1]] == 0:
                    ...
                    light_offsets(y + o[0], x + o[1], grid_, zeroed)


def increment(grid_, zeroed):
        ...
        for x, val in enumerate(row):
            if (y, x) not in zeroed:
                ...
                if grid_[y][x] == 0:
                    total_flashes += 1
                    zeroed.add((y, x))
                    light_offsets(y, x, grid_, zeroed)
    return grid_
...
for iteration_number in range(1000):
    zeroed.clear()
    grid_ = increment(grid_, zeroed)
    if iteration_number == 99:
        print("part1 answer", total_flashes)
